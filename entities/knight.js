class Knight {
    ...
    checkCollisions() {
        this.game.entities.forEach((entity) => {
            ...
            if (entity.boundingBox && entity instanceof Skeleton && entity.state != 4) {
                ...
                if (!verticalBox.collide(entity.boundingBox) && !horizontalBox.collide(entity.boundingBox)) {
                    entity.currSpeed = entity.maxSpeed;
                }
            }

            if (entity instanceof Skeleton) {
                if (this.hitBox && entity.hurtBox && this.hitBox.collide(entity.hurtBox)) {
                    if (entity.state != 2) {
                        entity.state = 3;
                    }
                    entity.health -= 100 * this.game.clockTick;
                } else if (this.hurtBox && entity.hitBox && this.hurtBox.collide(entity.hitBox)) {
                    if (this.state != 2) {
                        this.state = 3;
                    }
                    this.health -= 100 * this.game.clockTick;
                }
            }
    ...
    updateHitBox() {
        var current_frame = this.animations[this.state][this.direction].currentFrame();
        if (this.state == 2 && current_frame != 2 && current_frame != 3 && current_frame != 6) {
            if (this.direction == 0) {
